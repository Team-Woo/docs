...
| query?    | `string` | `null`    | Query string params found on the /bot/:ID/vote page. Example: ?a=1&b=2.                             |
...
| user   | `snowflake` | Discord ID of the user who voted.                                                                   |
| type   | `string`    | The type of the vote (should always be `"upvote"` except when using the test button it's `"test"`). |
| query? | `string` | `null`    | Query string params found on the /servers/:ID/vote page. Example: ?a=1&b=2.                         |
